  ...
  >
    <div class="space-y-5">
      ...
      <div>
        ...
        <div v-if="formData.removeCoverImage && !formData.newCoverImagePreview" class="mb-3">
          <div class="w-full h-48 bg-gray-100 rounded-lg border-2 border-dashed border-gray-300 flex flex-col items-center justify-center gap-3">
            <svg class="w-12 h-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
            </svg>
            <p class="text-gray-600 font-semibold">Cover image will be removed</p>
            <button
              @click="undoRemoveCoverImage"
              type="button"
        ...
        <!-- Upload Button -->
        <div v-if="!formData.newCoverImagePreview && (!currentCoverImage || formData.removeCoverImage)">
          <input
            ref="fileInputRef"
            type="file"
            accept="image/*"
          ...
          >
            ...
            {{ formData.removeCoverImage || !currentCoverImage ? 'Upload New Cover Image' : 'Change Cover Image' }}
          </button>
        </div>
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (formData.value.removeCoverImage) {
      formDataToSend.append('coverImage', 'null');
