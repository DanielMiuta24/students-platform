<template>
  <div class="flex flex-wrap gap-3">
    ...
    <button
      @click="selectStatus('StudySeeker')"
      :class="[
        'px-4 py-2 rounded-full font-semibold transition flex items-center gap-2',
    ...
    <button
      @click="selectStatus('Admin')"
      :class="[
        'px-4 py-2 rounded-full font-semibold transition flex items-center gap-2',
        selectedStatus === 'Admin'
...
const emit = defineEmits<{
  (e: 'change', status: UserStatus): void;
}>();

const selectedStatus = ref<UserStatus>(null);
