...
The user enters their email address in the provided field.
The system checks if the email exists in the database.
If the email exists:
The system sends a password reset email with a unique link.
The user checks their email and clicks on the password reset link.
The system displays the password reset form.
The user enters a new password and submits.
The system updates the user's password and confirms success.
...
If verification is required, the message includes next steps (e.g., "Check your email to verify the edit.").
